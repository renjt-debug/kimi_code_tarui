 ...
 *
 * 显示（与 ZCode 侧实时 TPS 胶囊同一版面）：
 *   主值 = 模型生成速度（生成中为滑动窗口估算，定稿后为权威口径）；
 *   副行只给三项 —— 整轮输出、生成时间、TTFT（静置超 60s 追加"上一轮"）。
 *   **悬停提示已取消**（title 恒为空），任何说明性文案都不要往界面上加。
 *
 * 口径（两边共用同一套规则）：
 *   - 模型生成速度 = 本次调用真实 output ÷ 生成阶段时长。分母排除这次调用的
 *     首 token 等待，但保留同一次生成内部的停顿；不含工具执行时间。
 *     分母取服务端 llmStreamDurationMs（权威，原样采用，不再减 TTFT），
 *     缺失时才退回「客户端首帧→末帧」的观测跨度，见 generationMs()。
 *   - 计时铁律：**没有工具事件就绝不因"一段时间没收到增量"而扣时** —— 那段
 *     可能是模型/网络的真实停顿，扣掉会让速率虚高，而界面已无告警可察。
 *     未知空档一律留在分母里（速率偏低但可解释）。
 *   - 生成阶段短于 minGenMs 不报速率，其 token 也不计入整轮累计（分子分母同进同出）。
 *
