...
// ---------------------------------------------------------------- 2. 实时窗口
{
  ...
  // 每 200ms 到达 20 个汉字；首帧仅作基准，后四帧为 80tok/800ms。
  for (let i = 0; i < 5; i++) {
    if (i) clock += 200;
  ...
  check('副行带实时估算与首 token', /实时估算/.test(live.sub) && /首 token/.test(live.sub), live.sub);
}
...
// ------------------------------------------------------------ 3. 定稿与自校准
{
  ...
  check('定稿副行 = output/耗时', /60 tok \/ 1\.50s/.test(fin.sub), fin.sub);
  check('TTFT 用服务端自报值', /TTFT 320ms\(服务端\)/.test(fin.sub), fin.sub);
  check('定稿副行带服务端解码耗时', /服务端解码 1\.18s/.test(fin.sub), fin.sub);
  ...
  // 第 2 步：同样文本，校准后的区间速率应为 80*0.6/0.8 = 60
  ws.emit(ev('turn.step.started', {}));
  for (let i = 0; i < 5; i++) {
    if (i) clock += 200;
    ws.emit(ev('assistant.delta', { delta: '一二三四五六七八九十壹贰叁肆伍陆柒捌玖拾'.slice(0, 20) }));
...
// assistant.delta，增量全在 transcript.ops 的 append op 里。
{
  ...
  check('等待期显示本轮已出 token', /本轮已出 12 tok/.test(waiting2.sub), waiting2.sub);
  ...
  check('定稿副行含 TTFT(服务端) 与服务端解码',
    /TTFT 22\.39s\(服务端\)/.test(fin.sub) && /服务端解码 822ms/.test(fin.sub), fin.sub);
  check('定稿副行含本步真实 token 与耗时', /22 tok \/ 823ms/.test(fin.sub), fin.sub);
...
function makeReplay() {
  clock = 0;
  const sb = makeSandbox();
  const ws = new sb.WebSocket('ws://localhost/api/v1/ws');
  let seq = 0;
  return {
    sb, ws,
    send(ops, session = 'a') {
...

{
  ...
  check('每500ms出50token显示100tok/s', near(r.sb.__kimiTps.state().window.tps, 100));
  clock += 1000;
  r.sb.drain();
  check('停顿一秒后速率降为50tok/s', near(r.sb.__kimiTps.state().window.tps, 50));
...

{
  ...
  check('满窗口的左端点不重复计数', w.span === 3000 && w.tok === 300 && near(w.tps, 100), JSON.stringify(w));
}
...

{
  ...
  r.sb.drain();
  check('新轮收到用量后显示正确数值', /本轮已出 12 tok/.test(snap(r.sb).sub), snap(r.sb).sub);
  r.send([{ op: 'turn.upsert', turn: { turnId: 't2', state: 'running' } }]);
  check('同轮running更新不清除当前用量', /本轮已出 12 tok/.test(snap(r.sb).sub));
  r.send([completed('t2.1'), running('t2.2', 't2')]);
  check('同轮进入下一步保留本轮累计用量', /本轮已出 12 tok/.test(snap(r.sb).sub));
