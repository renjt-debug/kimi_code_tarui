...
- **权威定稿**：同一条 op 还带 `step.timing.*`，于是能显示和 TUI 同口径的真实
  ...
  整轮累计（分子分母同进同出）。
- **两个口径**（与 ZCode 皮肤管理器的实时 TPS 胶囊一致，鼠标悬停可见公式）：
  - **模型生成速度** = 本次调用真实 `output` ÷ **生成阶段时长**。分母排除这次
    ...
    没给时才退回「客户端首帧 → 末帧」的观测跨度。
  - **整轮完成效率** = 整轮各次调用真实 `output` 之和 ÷ **整轮墙钟**（`turn.started`
    或首个 step 开始 → 现在），含每次调用的首 token 等待与工具执行。它描述端到端
    成本，必然不高于模型生成速度。
...
- **常驻**（`CFG.persistent`，与 ZCode 侧胶囊一致）：浮层**不自动隐藏**，任何
  时候都摆在那里，所以「鼠标悬停看两个口径的公式」这件事永远成立。过了 60 秒
  新鲜期不消失，只在副行标注「上一轮」；整轮墙钟也在回合静置超过 60 秒后**冻结**
  在最后一次活动时刻，否则放一晚上回来会看到整轮效率被稀释成 0.9 tok/s 这种
  没意义的数字。常驻下仍保留低频重绘（1.5s 一次），否则「上一轮」标记和悬停标题
  再也不会更新；唯一隐藏途径是 **Ctrl+Alt+T**。
- **悬停详情**：浮层任意阶段都有内容——实时阶段给「实时估算」口径，定稿后给
  「模型生成速度 + 整轮完成效率」两条算式，等首 token 与无数据时给口径说明，
  绝不会出现悬停空白（那会让人以为脚本没工作）。
- **自检**：[src/tps.test.cjs](src-tauri/src/tps.test.cjs) 用 `vm` 造了假浏览器 +
  ...
  以及第 11/12 节的口径对齐与常驻悬停回归（共 125 条断言）。
